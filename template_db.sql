...
CREATE TABLE `os_info`  (
  `id` int(0) NOT NULL AUTO_INCREMENT,
  `shr_id` int(0) NOT NULL,
  `os_version` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL,
  PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS POC (
    ...
    Vul_name VARCHAR(600) NOT NULL,
    Type VARCHAR(255),
    Description TEXT,
    Affected_infra TEXT NOT NULL,
    Cvss_score TEXT , 
    Poc_condition VARCHAR(20) NOT NULL CHECK (Poc_condition IN ('有', '暂存', '无')),
    Script_type VARCHAR(20) NOT NULL,  -- 禁止空值
    Script TEXT,
    Timestamp TEXT NOT NULL,
    CONSTRAINT chk CHECK (
        (Poc_condition IN ('有', '暂存') AND Script_type IN ('python', 'c/c++', 'yaml')) OR
        (Poc_condition = '无' AND Script_type = '无')
    ),
...
INSERT INTO POC VALUES (1,'CVE-2019-11510','Pulse Secure VPN 任意文件读取','代码注入','在8.1R15.1之前的脉冲安全脉冲连接安全（PCS），8.2R12.1之前的8.2之前，8.3R7.1之前的8.3和9.0R3.4之前的9.0，经过身份验证的攻击者可以上传恶意文件以写入任意文件，因为不安全的权限。','待补充','10','有','python','','2024-05-12 12:54:23');
INSERT INTO POC VALUES (2,'CVE-2021-123324','SQL注入漏洞','SQL 注入','这是一个示例描述','0','0.0','有','c/c++','','2024-05-12 15:51:02');
INSERT INTO POC VALUES (3,'CVE-2020-15778','OpenSSH 操作系统命令注入漏洞','代码注入','OpenSSH 8.3p1及之前版本中的scp的scp.c文件存在操作系统命令注入漏洞。该漏洞即使在禁用ssh登录的情况下，但是允许使用scp传文件，而且远程服务器允许使用反引号(`)，可利用scp复制文件到远程服务器时，执行带有payload的scp命令，从而在后续利用中getshell。','OpenSSH','7.8','有','python','','2024-07-09 22:32:02');
INSERT INTO POC VALUES (4,'CVE-2016-3510','WebLogic T3 协议反序列化漏洞','代码注入','CVE-2016-3510漏洞是对CVE-2015-4852漏洞修复的绕过，攻击者在可以通过该漏洞实现远程命令执行。','WebLogic','9.8','有','python','Weblogic_CVE_2017_10271_RCE_test.py','2024-07-23 17:21:02');
INSERT INTO POC VALUES (5,'CVE-2018-6789','Exim 远程命令执行漏洞','远程代码执行 (RCE)','2018年2月，流行的开源邮件服务器Exim曝出了堆溢出漏洞（CVE-2018-6789），几乎影响了4.90.1之前的所有版本。可以利用该漏洞绕过各种缓解措施成功达成远程代码执行：','Exim','9.8','有','python','CVE_2018_6789.py','2024-07-23 21:39:02');
INSERT INTO POC VALUES (6,'CVE-2017-12617','Apache Tomcat PUT文件上传漏洞','远程代码执行 (RCE)','如果配置了默认servlet，则在9.0.1（Beta），8.5.23,8.0.47和7.0.82之前的所有Tomcat版本都包含所有操作系统上的潜在危险的远程执行代码（RCE）漏洞，CVE-2017-12617：远程代码执行漏洞。只需参数readonly设置为false或者使用参数readonly设置启用WebDAV servlet false。此配置将允许任何未经身份验证的用户上传文件（如WebDAV中所使用的）。只要JSP可以上传，然后就可以在服务器上执行。在一定条件下，攻击者可以利用这两个漏洞，获取用户服务器上 JSP 文件的源代码，或是通过精心构造的攻击请求，向用户服务器上传恶意JSP文件，通过上传的 JSP 文件 ，可在用户服务器上执行任意代码，从而导致数据泄露或获取服务器权限，存在高安全风险。','Apache Tomcat','8.1','有','python','CVE_2017_12617.py','2024-07-24 14:07:02');
INSERT INTO POC VALUES (7,'CVE-2017-12615','Apache Tomcat PUT 远程命令执行漏洞','远程代码执行 (RCE)','当 Tomcat 运行在 Windows 主机上，且启用了 HTTP PUT 请求方法（例如，将readonly 初始化参数由默认值设置为 false），攻击者将有可能可通过精心构造的攻击请求向服务器上传包含任意代码的 JSP 文件。之后，JSP 文件中的代码将能被服务器执行。','Apache Tomcat','8.1','有','python','CVE_2017_12615.py','2024-07-24 14:08:33');
INSERT INTO POC VALUES (8,'CVE-2017-10271','WebLogic XMLDecoder反序列化漏洞（CVE-2017-10271）','代码注入','Weblogic的WLS Security组件对外提供webservice服务，其中使用了XMLDecoder来解析用户传入的XML数据，在解析的过程中出现反序列化漏洞，导致可执行任意命令。','WebLogic','7.5','有','python','Weblogic_CVE_2017_10271_RCE_test.py','2024-07-24 15:51:33');
INSERT INTO POC VALUES (9,'CVE-2019-10149','Exim 远程代码执行漏洞','远程代码执行 (RCE)','在&nbsp;Exim versions 4.87 to 4.91（含）中发现了一个漏洞。 /src/deliver.c中的deliver_message（）函数中的收件人地址验证不正确可能导致远程命令执行。','Exim','9.8','暂存','python','','2024-07-25 16:51:30');
INSERT INTO POC VALUES (10,'CVE-2019-15107','Webmin <=1.920 password_change.cgi 远程命令执行漏洞','远程代码执行 (RCE)','Webmin是一套基于Web的用于类Unix操作系统中的系统管理工具。Webmin 1.920及之前版本中的password_change.cgi存在命令注入漏洞。该漏洞源于外部输入数据构造可执行命令过程中，网络系统或产品未正确过滤其中的特殊元素。攻击者可利用该漏洞执行非法命令。','Webmin','9.8','暂存','python',NULL,'2024-07-25 16:53:30');
INSERT INTO POC VALUES (11,'CVE-1234-test','Test Vulnerability','远程代码执行 (RCE)','This is a test vulnerability description.','0','8.1','暂存','python','','2024-07-20 18:51:06');



-- 导入第一个 CSV 文件
LOAD DATA INFILE '/var/lib/mysql-files/poc_data1.csv'
INTO TABLE POC
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n'
IGNORE 1 ROWS
(Vuln_id, Vul_name, Type, Description, Affected_infra, Cvss_score, Poc_condition ,Script_type, @script, @dummy)
SET Script = '',
    Timestamp = NOW();

-- 导入第二个 CSV 文件
LOAD DATA INFILE '/var/lib/mysql-files/poc_data2.csv'
INTO TABLE POC
FIELDS TERMINATED BY ',' 
ENCLOSED BY '"'
LINES TERMINATED BY '\n'
