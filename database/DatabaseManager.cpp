...
//
////添加POC数据
//bool DatabaseManager::insertData(const std::string& vuln_id, const std::string& vul_name, const std::string& type, const std::string& description, const std::string affected_infra , const std::string cvss_score, const std::string poc_condition, const std::string& script_type, const std::string& script) {
//    std::string timestamp = getCurrentTimestamp();
//    std::string sql = "INSERT INTO POC (Vuln_id, Vul_name, Type, Description, Affected_infra ,Cvss_score,Poc_condition, Script_type, Script, Timestamp) VALUES (?, ?, ?, ?, ?,?,?, ?, ?, ?);";
//    //表示一个编译好的SQL语句
//    sqlite3_stmt* stmt;
...
//    //更新操作
//    std::string timestamp = getCurrentTimestamp();
//    std::string sql = "UPDATE POC SET Vuln_id = ?, Vul_name = ?, Type = ?, Description = ?, Affected_infra = ?,Cvss_score = ?,Poc_condition = ?, Script_type = ?, Script = ?, Timestamp = ? WHERE ID = ?;";
//
//    sqlite3_stmt* stmt;
//    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
//        system_logger->error("SQL error: {}", sqlite3_errmsg(db));
...
//    sqlite3_bind_text(stmt, 6, poc.Cvss_score.c_str(), -1, SQLITE_TRANSIENT);
//    sqlite3_bind_text(stmt, 7, poc.Poc_condition.c_str(), -1, SQLITE_TRANSIENT);
//    sqlite3_bind_text(stmt, 8, poc.script_type.c_str(), -1, SQLITE_TRANSIENT);
//    sqlite3_bind_text(stmt, 9, poc.script.c_str(), -1, SQLITE_TRANSIENT);
